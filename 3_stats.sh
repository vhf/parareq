...
s200=$(grep -oE '^\d{1,}\t200' ./output/2_good | wc -l | column -t | cut -d' ' -f1)
s200xml=$(grep -oE '^\d{1,}\t200.*\t.*\t.*xml.*$' ./output/2_good | wc -l | column -t | cut -d' ' -f1)
s200xmlu=$(grep -oE '^\d{1,}\t200.*\t.*\t.*xml.*$' ./output/2_good | sort -u | wc -l | column -t | cut -d' ' -f1)
